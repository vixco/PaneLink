...
test('detects h264 stream endpoints separately from png frames', () => {
  assert.equal(isH264StreamEndpoint('http://192.168.1.20:48172/h264?fps=60'), true);
...
test('rewrites loopback h264 endpoint to the remote control host', () => {
  assert.equal(
    endpointForControlHost('http://127.0.0.1:48172/h264?fps=60', 'http://192.168.1.20:48170'),
    'http://192.168.1.20:48172/h264?fps=60',
  );
});
